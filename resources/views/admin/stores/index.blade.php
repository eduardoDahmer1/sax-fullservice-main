    ...
    <div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <!-- Modal body -->
                <div class="modal-body">
                    <p class="text-center">{{ __('You are about to delete this Store.') }}</p>
...
@section('scripts')
    <script type="text/javascript">
        var table = $('#geniustable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin-stores-datatables') }}',
            columns: [{
                data: 'action',
                searchable: false,
                orderable: false
            ...
            initComplete: function(settings, json) {
                @if (config('features.multistore'))
                    $(".btn-area").append('<div class="col-sm-4 text-right">' +
                        '<a class="add-btn" data-href="{{ route('admin-stores-create') }}" data-header="{{ __('Add New Store') }}" id="add-data" data-toggle="modal" data-target="#modal1">' +
                        '<i class="fas fa-plus"></i> {{ __('Add New Store') }}' +
                        '</a>' +
                        '</div>');
                @endif
                /*
