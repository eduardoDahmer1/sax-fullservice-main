            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <ul class="links">
                        ...
                        <li>
                            <a href="{{ route('admin-team_member-index') }}">{{ __('Team') }}</a>
                        </li>
                        <li>
                            <a href="{{ route('admin-team_member-index') }}">{{ __('Team Members') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        @include('includes.admin.partials.team-member-tabs')
    ...
    <div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <!-- Modal body -->
                <div class="modal-body">
                    <p class="text-center">{{ __('You are about to delete this team member.') }}</p>
...
@section('scripts')
    ...
    <script type="text/javascript">
        var table = $('#geniustable').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('admin-team_member-datatables') }}',
            columns: [{
                    data: 'action',
                    searchable: false,
                    orderable: false
            ...
            initComplete: function(settings, json) {
                $(".btn-area").append('<div class="col-sm-4 table-contents">' +
                    '<a class="add-btn" data-href="{{ route('admin-team_member-create') }}" data-header="{{ __('Add New Team Member') }}" id="add-data" data-toggle="modal" data-target="#modal1">' +
                    '<i class="fas fa-plus"></i> {{ __('Add New Team Member') }}' +
                    '</a>' +
                    '</div>');
                /*
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', 'a', function(e) {
                ...
                if (!(link.attr("data-href") || link.attr("href").indexOf("#") > -1 || link.attr("href")
                        .indexOf("javascript") > -1 || !(link.attr("href").indexOf("team_member")))) {
                    sessionStorage.setItem("CurrentPage", 0);
                    table.state.clear();
                }
            });
