...
@section('content')
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __('Vendor Details') }} <a class="add-btn" href="{{ url()->previous() }}"><i
                                ...
                                <div class="col-md-4">
                                    <div class="user-image">
                                        @if ($data->is_provider == 1)
                                            <img src="{{ $data->photo ? asset($data->photo) : asset('assets/images/noimage.png') }}"
                                                alt="No Image">
                                        @else
                                            <img src="{{ $data->photo ? asset('storage/images/users/' . $data->photo) : asset('assets/images/noimage.png') }}"
                                                alt="{{ __(' No Image') }}">
                                        @endif
                                    ...
                                    <div class="table-responsive show-table">
                                        <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $data->vendor_corporate_name }}</td>
                                            </tr>
                                            <tr>
                                                <th>{{ __('CNPJ') }}</th>
                                                <td>{{ $data->vendor_document }}</td>
                                            </tr>
                                            <tr>
                                                <th>{{ __('Phone') }}</th>
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $data->vendor_payment_methods }}</td>
                                            </tr>
                                            <tr>
                                                <th>{{ __('Delivery Info') }}</th>
                                                <td>{{ $data->vendor_delivery_info }}</td>
                                            </tr>
                                            <tr>
                                                <th>{{ __('Email') }}</th>
                                                <td>{{ $data->email }}</td>
                                ...
                                <div class="col-md-4">
                                    <div class="table-responsive show-table">
                                        <table class="table">
                                            ...
                                            <tr>
                                                <th>{{ __('Use Maps') }}</th>
                                                <td>{{ $data->vendor_map_embed ? __('Yes') : __('No') }}</td>
                                            </tr>
                                            <tr>
                                                <th>{{ __('Joined') }}</th>
                                            ...
                                            <tr>
                                                <td>
                                                    @if ($data->checkStatus())
                                                        ...
                                                        <a class="set-gallery1" href="javascript:;" data-toggle="modal"
                                                            data-target="#setgallery"><input type="hidden"
                                                                value="{{ $data->verifies()->where('status', '=', 'Verified')->first()->id }}">({{ __('View') }})</a>
                                                    @else
                                                        <a class="badge badge-danger verify-link"
                                                            href="javascript:;">{{ __('Unverified') }}</a>
                                                    @endif
                                                </td>
                                                <td>
                                                </td>
                                        ...
                                        <table id="example2" class="table table-hover dt-responsive" cellspacing="0"
                                            ...
                                            <tbody>
                                                @foreach ($data->products as $dt)
                                                    <tr>
                                                        <td><a href="{{ route('front.product', $dt->slug) }}"
                                                                target="_blank">{{ sprintf("%'.08d", $dt->id) }}</a></td>
                                                        <td>{{ $dt->type }}</td>
                                                        @php
                                                            $stck = (string) $dt->stock;
                                                            if ($stck == '0') {
                                                                $stck = __('Out of Stock!');
                                                            } elseif ($stck == null) {
                                                                $stck = __('Unlimited');
                                                            }
                                                        @endphp
                                                        ...
                                                        <td>
                                                            <div class="action-list">
                                                                <select
                                                                    class="process select droplinks {{ $dt->status == 1 ? 'drop-success' : 'drop-danger' }}">
                                                                    <option data-val="1"
                                                                        value="{{ route('admin-prod-status', ['id1' => $data->id, 'id2' => 1]) }}"
                                                                        {{ $dt->status == 1 ? 'selected' : '' }}>
                                                                        {{ __('Activated') }}</option>
                                                                    <<option data-val="0"
                                                                        value="{{ route('admin-prod-status', ['id1' => $data->id, 'id2' => 0]) }}"
                                                                        {{ $dt->status == 0 ? 'selected' : '' }}>
                                                                        {{ __('Deactivated') }}</option>
                                                                </select>
                                                            </div>
                                                        </td>
                                                        <td>
                                                            <a href=" {{ route('admin-prod-edit', $dt->id) }}"
                                                                class="view-details">
