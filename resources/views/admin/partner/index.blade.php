...
@section('content')
    <div class="content-area">
        <div class="mr-breadcrumb">
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __('Logo Slider') }}</h4>
                    <ul class="links">
                        <li>
                            <a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a>
                        </li>
                        <li>
                            <a href="{{ route('admin-partner-index') }}">{{ __('Logo Slider') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        ...
                                        <th><i class="fa fa-camera fa-lg" data-toggle="tooltip"
                                                title='{{ __('Featured Photo') }}'></i></th>
                                        <th><i class="fa fa-link fa-lg" data-toggle="tooltip"
                                                title='{{ __('Link') }}'></i>
                                        </th>
                                    </tr>
                                </thead>
    ...
    <div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <!-- Modal body -->
                <div class="modal-body">
                    <p class="text-center">{{ __('You are about to delete this Logo.') }}</p>
        ...
        var table = $('#geniustable').DataTable({
            ...
            initComplete: function(settings, json) {
                $(".btn-area").append('<div class="col-sm-4 table-contents">' +
                    '<a class="add-btn" data-href="{{ route('admin-partner-create') }}" data-header="{{ __('Add New Logo') }}" id="add-data" data-toggle="modal" data-target="#modal1">' +
                    '<i class="fas fa-plus"></i> {{ __('Add New Logo') }}' +
                    '</a>' +
                    '</div>');
                /*
