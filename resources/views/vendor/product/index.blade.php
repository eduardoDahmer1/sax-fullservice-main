...
@section('scripts')
    ...
    <script type="text/javascript">
        var table = $('#geniustable').DataTable({
            ...
            drawCallback: function(settings) {
                $(".checkboxStatus").on('click', function() {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: '{{ url('vendor/products/status') }}' + '/' + id + '/' +
                            statusNovo
                    });
                });
            }
        ...
        var table2 = $('#geniustable2').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '{{ route('vendor-prod-datatablesadmin') }}',
            columns: [{
                    data: 'photo',
                    name: 'photo',
                    render: function(data) {
    ...
    <script type="text/javascript">
        ...
        $(document).on('click', '.copyproduct', function() {
            ...
            $('#copy_modal').find('.modal-title').html($(this).attr('data-header'));
            $('#copy_modal .modal-content .modal-body').html('').load($(this).attr('data-href'), function(response,
                status,
                xhr) {
                if (status == "success") {
                    if (admin_loader == 1) {
