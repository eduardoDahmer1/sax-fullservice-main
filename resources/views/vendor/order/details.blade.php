            ...
            <div class="row">
                <div class="col-lg-12">
                    <h4 class="heading">{{ __('Order Details') }} <a class="add-btn"
                            href="{{ route('vendor-order-index') }}"><i class="fas fa-arrow-left"></i> {{ __('Back') }}</a>
                    </h4>
                    <ul class="links">
                        <li>
                        ...
                        <li>
                            <a href="javascript:;">{{ __('All Orders') }}</a>
                        </li>
                        <li>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td width="10%">:</td>
                                        <td width="45%">
                                            {{ $order->vendororders()->where('user_id', '=', $user->id)->sum('qty') }}</td>
                                    </tr>
                                    ...
                                    @if (Auth::user()->id == $order->vendor_shipping_id)
                                        @if ($order->shipping_cost != 0)
                                            ...
                                            @endphp
                                            @if (Shipping::where('price', '=', $price)->count() > 0)
                                                <tr>
                                                    <th width="45%">
                                                        {{ Shipping::where('price', '=', $price)->first()->title }}</th>
                                                    <td width="10%">:</td>
                                                    <td width="45%">
                                    ...
                                    @if (Auth::user()->id == $order->vendor_packing_id)
                                        @if ($order->packing_cost != 0)
                                            ...
                                            @endphp
                                            @if (Package::where('price', '=', $pprice)->count() > 0)
                                                <tr>
                                                    <th width="45%">
                                                        {{ Package::where('price', '=', $pprice)->first()->title }}</th>
                                                    <td width="10%">:</td>
                                                    <td width="45%">
                                                ...
                                                <tr>
                                                        ...
                                                        @if ($product['item']['user_id'] != 0)
                                                            ...
                                                            @else
                                                                @if ($user->status == 'pending')
                                                                    <span
                                                                        class="badge badge-warning">{{ __(ucwords($user->status)) }}</span>
                                                                @elseif($user->status == 'processing')
                                                                    <span
                                                                        class="badge badge-info">{{ __(ucwords($user->status)) }}</span>
                                                                @elseif($user->status == 'on delivery')
                                                                    <span
                                                                        class="badge badge-primary">{{ __(ucwords($user->status)) }}</span>
                                                                @elseif($user->status == 'completed')
                                                                    <span
                                                                        class="badge badge-success">{{ __(ucwords($user->status)) }}</span>
                                                                @elseif($user->status == 'declined')
                                                                    <span
                                                                        class="badge badge-danger">{{ __(ucwords($user->status)) }}</span>
                                                                @endif
                                                            @endif
                                                        @endif
                                                    </td>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if ($product['item']['user_id'] != 0)
                                                            ...
                                                            @else
                                                                <a
                                                                    href="javascript:;">{{ mb_strlen($product['item']['name'], 'utf-8') > 30
                                                                        ? mb_substr($product['item']['name'], 0, 30, 'utf-8') . '...'
                                                                        : $product['item']['name'] }}</a>
                                                            @endif
                                                        @endif
                                                        ...
                                                        @if ($product['license'] != '')
                                                            <a href="javascript:;" data-toggle="modal"
                                                                ...
                                                                style="padding: 5px 12px;"><i class="fa fa-eye"></i> View
                                                                License</a>
                                                        @endif
                                                    ...
                                                    <td>
                                                        ...
                                                        <p>
                                                            <strong>{{ __('Quantity') }} :</strong> {{ $product['qty'] }}
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p class="text-center">{{ __('The Licenes Key is') }} : <span id="key"></span> <a
                            href="javascript:;" id="license-edit">{{ __('License Key') }}</a><a href="javascript:;"
                            id="license-cancel" class="showbox">{{ __('Cancel') }}</a></p>
                    <form method="POST" action="{{ route('vendor-order-license', $order->order_number) }}"
                        id="edit-license" style="display: none;">
                        {{ csrf_field() }}
                        <input type="hidden" name="license_key" id="license-key" value="">
                        <div class="form-group text-center">
                            <input type="text" name="{{ __(' Save License') }}"
                                placeholder="{{ __(' Enter New License Key') }}" style="width: 40%; border: none;"
                                required=""><input type="submit" name="submit" value="Save License"
                                            ...
                                            <ul>
                                                <li>
                                                    <input type="email" class="input-field eml-val" id="eml"
                                                        name="to" placeholder="{{ __(' Email Address') }} *"
                                                        value="" required="">
                                                </li>
                                                <li>
