...
class VerificationController extends Controller
{
    ...
    {
        if ($status == 'all') {
            $datas = Verification::orderBy('created_at', 'DESC');
        }

        if ($status == 'pending') {
            $datas = Verification::where('status', '=', 'Pending');
        }

        if ($status == 'verified') {
            $datas = Verification::where('status', '=', 'Verified');
        }

        if ($status == 'declined') {
            $datas = Verification::where('status', '=', 'Declined');
        }
        return Datatables::of($datas)
            ->addColumn('name', function (Verification $data) {
                $name = isset($data->user->owner_name) ? '<a href="' . route('admin-vendor-show', $data->user_id) . '" target="_blank">' . $data->user->owner_name . '</a>' : __('Removed');
                return  $name;
            })
            ...
            ->addColumn('status', function (Verification $data) {
                ...
                return '<div class="action-list"><select class="process select vendor-droplinks ' . $class . ' disabled">' .
            ...
            ->addColumn('action', function (Verification $data) {
                return '<div class="action-list"><a href="javascript:;" class="set-gallery-verify" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="' . $data->id . '"><i class="fas fa-paperclip"></i> ' . __('View Attachments') . '</a></div>';
            })
    ...
    public function index()
    {
        ...
        return view('admin.verify.index', compact('filters'));
    }
