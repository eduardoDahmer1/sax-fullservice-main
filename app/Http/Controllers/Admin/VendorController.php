...
class VendorController extends Controller
{
    ...
    public function subsdatatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('created_at', function(UserSubscription $data) {
                                \Carbon\Carbon::setLocale($this->lang->locale);
                                $date = $data->created_at->diffForHumans();
                                return $date;
                            }) 
                            ->addColumn('action', function(UserSubscription $data) {
                                return '<div class="action-list"><a data-href="' . route('admin-vendor-sub',$data->id) . '" class="view details-width" data-toggle="modal" data-target="#modal1"> <i class="fas fa-eye"></i>'.__('Details').'</a></div>';
                            }) 
                            ->rawColumns(['action'])
                            ->toJson(); //--- Returning Json Data To Client Side
    ...
    public function sub($id)
    {
        ...
        $days_left = Carbon::parse($subs->created_at->addDays($subs->days))->format('d/m/Y');
        return view('admin.vendor.subscription-details',compact('subs', 'days_left'));
    }
    ...
    public function verifySubmit(Request $request, $id)
    {
        ...
        $input = $this->withRequiredFields($request->all(), ['details']);

        $info = [
            'admin_warning' => 1,
            "{$this->lang->locale}" => [
                'warning_reason' => $input[$this->lang->locale]['details']
            ]
        ];
        foreach ($this->locales as $loc) {
            ...
            $info[$loc->locale]['warning_reason'] = $input[$loc->locale]['details'];
        }

        $user->verifies()->create($info);
    ...
    public function update(Request $request, $id)
    {
        ...
        $customs = [
            'shop_name.unique' => __('Shop Name has already been taken. Choose another name'),
        ];
        ...
        $data = $request->all();
        $user->update($data);
        $msg = __('Vendor Information Updated Successfully.');
        return response()->json($msg);   
    }
    ...
    public function destroy($id)
    {
        ...
        if($user->notivications->count() > 0)
        {
            ...
            //--- Redirect Section     
            $msg = __('Vendor deleted successfully.');
            return response()->json($msg);      
            //--- Redirect Section Ends    
    }
        ...
        public function withdrawdatatables()
        {
             ...
             return Datatables::of($datas)
                                ...
                                ->editColumn('status', function(Withdraw $data) {
                                    //add the status strings to be able to translate them
                                    $statusStr = [
                                        "pending" => __("Pending"),
                                        "completed" => __("Completed"),
                                        "rejected" => __("Rejected"),
                                    ];
                                    $status = __(ucfirst($data->status));
                                    return $status;
                                }) 
                                ...
                                ->addColumn('action', function(Withdraw $data) {
                                    $action = '<div class="action-list"><a data-href="' . route('admin-vendor-withdraw-show',$data->id) . '" class="view" data-toggle="modal" data-target="#modal1"> <i class="fas fa-eye"></i> '.__('Details').'</a>';
                                    if($data->status == "pending") {
                                    $action .= '<a data-href="' . route('admin-vendor-withdraw-accept',$data->id) . '" data-toggle="modal" data-target="#confirm-delete"> <i class="fas fa-check"></i>'. __('Accept') .'</a><a data-href="' . route('admin-vendor-withdraw-reject',$data->id) . '" data-toggle="modal" data-target="#confirm-delete1"> <i class="fas fa-trash-alt"></i>'.  __('Reject') .'</a>';
                                    }
                                    $action .= '</div>';
                                    return $action;
                                }) 
