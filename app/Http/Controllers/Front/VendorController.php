...
class VendorController extends Controller
{
    ...
    public function index(Request $request,$slug)
    {
        ...
        $data['vendor'] = $vendor;
        $services_vendor = Service::where('user_id','=',$vendor->id)->get();
        ...
        $range_max = Product::max('price') * 1.5;
        ...
        $categories = Category::orderBy('slug')->where('status',1)->get();
        $data['categories'] = $categories;

        return view('front.vendor', $data, compact('services_vendor'));
    }

    public function stores(){
        
        // get unique first letter from brands to use in alphabetical sort
        $chars = User::where('is_vendor', 2)->where('shop_name', '!=', null)->where('is_vendor_verified', 1)->selectRaw('LEFT(UPPER(shop_name),1) AS fl')->orderBy('fl', 'ASC')->groupBy('fl')->pluck('fl');
        
        $vendor_stores = User::where('is_vendor', 2)->where('shop_name', '!=', null)->where('is_vendor_verified', 1)->withCount(['products' => function ($query) {
            $query->byStore();
        }])->orderBy('shop_name')->get();

        return view('front.stores', compact('chars', 'vendor_stores'));
    }
