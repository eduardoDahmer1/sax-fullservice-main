    ...
    public function store(Request $request)
    {
        ...
        public function payreturn()
        {
            if (Session::has('tempcart')) {
                ...
                $order->pay42_due_date = Session::get('pay42_pix_days_due_date');
                $order->pay42_due_date = date('d-m-Y H:i:s', strtotime($order->pay42_due_date));
            ...
            if (isset($order->is_qrcode)) {
                $pay42_qrcode = Session::get('pay42_pix_qrcode');
                Session::forget('pay42_pix_qrcode');
                return view('front.success', compact('tempcart', 'order', 'first_curr', 'pay42_qrcode', 'order_curr', 'bank_accounts'));
            }
            return view('front.success', compact('tempcart', 'order', 'first_curr', 'order_curr', 'bank_accounts'));
        }

        public function payreturnSimplifidCheckout()
        {
            if (Session::has('tempcart')) {
                $oldCart = Session::get('tempcart');
                $tempcart = new Cart($oldCart);
